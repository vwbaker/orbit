...
void CommandBufferManager::MarkCommandBufferEnd(const VkCommandBuffer& command_buffer) {
  ...
  // Even though, we are just under a reader mutex, writing to this field is safe, as
...
void CommandBufferManager::CompleteSubmits(const VkDevice& device) {
  ...
  for (const auto& completed_submission : completed_submissions) {
    ...
    const internal::SubmissionMetaInformation& meta_info = completed_submission.meta_information;
    orbit_grpc_protos::GpuQueueSubmissionMetaInfo* meta_info_proto =
        submission_proto.mutable_meta_info();
    meta_info_proto->set_thread_id(meta_info.thread_id);
    meta_info_proto->set_pre_submission_cpu_timestamp(meta_info.pre_submission_cpu_timestamp);
    meta_info_proto->set_post_submission_cpu_timestamp(meta_info.post_submission_cpu_timestamp);
    ...
    for (const auto& marker_state : completed_submission.completed_markers) {
      ...
      const auto& begin_meta_info = marker_state.begin_info->meta_information;
      orbit_grpc_protos::GpuDebugMarkerBeginInfo* begin_debug_marker_proto =
          marker_proto->mutable_begin_marker();
      orbit_grpc_protos::GpuQueueSubmissionMetaInfo* begin_meta_info_proto =
          submission_proto.mutable_meta_info();
      begin_meta_info_proto->set_thread_id(begin_meta_info.thread_id);
      begin_meta_info_proto->set_pre_submission_cpu_timestamp(
          begin_meta_info.pre_submission_cpu_timestamp);
      begin_meta_info_proto->set_post_submission_cpu_timestamp(
  ...
  for (auto& [queue, queue_submissions] : queue_to_submissions_) {
    ...
    while (submission_it != queue_submissions.end()) {
      ...
      while (submit_info_reverse_it != submission.submit_infos.rend()) {
        ...
        static constexpr VkDeviceSize result_stride = sizeof(uint64_t);
